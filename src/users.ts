...
/**
 * Derive the public subscription_status from the four computed booleans
 * the backend returns. Order matters: `active` (paying user) takes
 * precedence over a still-open trial / demo window because a renewed
 * subscription should not appear as "trial" once the first paid charge
 * lands. `expired` is the terminal state and the only one the worker
 * uses to deny chat access.
 */
